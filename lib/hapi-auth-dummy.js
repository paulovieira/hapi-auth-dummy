    ...
    const scheme = {
        ...
        authenticate: function (request, reply){
            ...
            strategyOptions.validateFunc(name, function (err, isValid, credentials) {
                ...
                // any other possibilities of authentication (this makes sense only when
                // the route has multiple strategies)
                // - if we don't include the error message AND include the scheme name, we
                // want to try the other strategies

                // - if we want the handler to always execute (the response can be given  
                //   without authentication - it will be adjusted), use the mode 'try'
                // - if we want the handler to execute only if authentication succeeded,
                //   (makes sense for API endpoints with private data), use mode 'required'
                ...
                // So if we use 'optional' mode, the handler would be executed in case a), but not in b)
                // If we use 'try' mode, the handler would be executed in both cases
